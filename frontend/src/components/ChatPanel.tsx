...
export default function ChatPanel({ messages, onSendMessage }: ChatPanelProps) {
  ...
  return (
    <div className="flex flex-col h-full w-full bg-[#0f172a] rounded-2xl border border-white/10 overflow-hidden shadow-2xl min-h-0">
      ...
      <div className="flex-1 overflow-y-auto p-4 space-y-4">
        {messages.map((msg) => (
          <div key={msg.id} className={`flex flex-col ${msg.isMine ? 'items-end' : 'items-start'}`}>
            ...
            <div className={`px-4 py-2 rounded-2xl max-w-[85%] text-sm shadow-sm ${msg.isMine
