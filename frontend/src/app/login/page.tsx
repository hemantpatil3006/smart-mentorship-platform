  ...
  return (
    <div className="min-h-screen bg-slate-950 flex items-center justify-center relative overflow-hidden p-6 font-sans">
      ...
      <div className="w-full max-w-md p-6 sm:p-10 bg-slate-900/50 backdrop-blur-xl rounded-xl sm:rounded-2xl shadow-2xl border border-white/10 relative z-10">
        ...
        <h2 className="text-2xl sm:text-3xl font-extrabold mb-1.5 sm:mb-2 text-center text-white tracking-tight">Welcome Back</h2>
        <p className="text-center text-slate-400 mb-6 sm:mb-8 text-xs sm:text-sm">Sign in to continue your journey.</p>
        ...
        <form onSubmit={handleLogin} className="space-y-5">
          ...
          <button
            type="submit"
            disabled={loading}
            className="w-full py-3 sm:py-3.5 px-4 mt-2 sm:mt-4 bg-gradient-to-r from-emerald-500 to-teal-600 text-white font-bold rounded-lg sm:rounded-xl hover:from-emerald-400 hover:to-teal-500 transition-all duration-300 shadow-[0_0_20px_rgba(16,185,129,0.3)] disabled:opacity-50 disabled:shadow-none text-sm sm:text-base"
          >
            {loading ? 'Authenticating...' : 'Sign In'}
          </button>
        </form>
        <p className="mt-6 sm:mt-8 text-center text-xs sm:text-sm text-slate-400">
          Don't have an account? <Link href="/signup" className="text-emerald-400 font-semibold hover:text-emerald-300 transition-colors whitespace-nowrap">Create one here</Link>
        </p>
      </div>
    </div>
  );
